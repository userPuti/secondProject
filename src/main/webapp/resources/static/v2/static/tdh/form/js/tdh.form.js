...
function createOptions($sel) {
    ...
    if ($sel[0].style.fontSize) {
        $list.css('font-size', $sel[0].style.fontSize);
    }
    var liHtml = '';
    if (checkall) {
...
function switchInit(obj) {
    ...
    switchBoxs.each(function () {
        var o = jq(this), name = o.attr('name'), txtstr = o.attr('data-text'), cAttr = o.attr('checked'),
            dAttr = o.attr('disabled'), checked, disabled;
